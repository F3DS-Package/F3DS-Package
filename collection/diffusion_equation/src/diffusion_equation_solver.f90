module diffusion_equation_module
    use typedef_module
    use vector_module
    use stdio_module
    ...
    real(real_kind) :: diffusion_coefficient
...
contains
    ...
    subroutine initialize_model(a_configuration)
        ...
        call a_configuration%get_real  ( "Model.Diffusion coefficient", diffusion_coefficient, found)
        if(.not. found) call call_error("'Model.Diffusion coefficient' is not found in the configuration file you set.")
    end subroutine initialize_model

    subroutine write_result(a_cellsystem, a_result_writer, conservative_variables_set, gradient_conservative_variables_set)
        ...
        real (real_kind        ), intent(inout) :: gradient_conservative_variables_set(:, :)
        call a_cellsystem%open_file   (a_result_writer)
        call a_cellsystem%write_scolar(a_result_writer, "Phi", conservative_variables_set)
        call a_cellsystem%write_vector(a_result_writer, "Gradient Phi", gradient_conservative_variables_set)
    ...
    pure function spectral_radius(variable, length) result(r)
        ...
        real   (real_kind) :: r
        r = (2.0_real_kind * diffusion_coefficient) / length
    end function

    subroutine flux_calculation_operator(flux, gradient_phi)
        real(real_kind), intent(inout) :: flux(:)
        real(real_kind), intent(in   ) :: gradient_phi(:)
        flux(:) = diffusion_coefficient * gradient_phi(:)
    end subroutine
...
program diffusion_equation_solver
    use typedef_module
    use vector_module
    use stdio_module
    use class_cellsystem
    ...
    real(real_kind), allocatable :: gradient_conservative_variables_set(:, :)
    ...
    call a_cellsystem%initialize(gradient_conservative_variables_set, flux_lenght)
    ...
    do while ( .not. a_cellsystem%satisfy_termination_criterion(a_termination_criterion) )
        ...
        if ( a_cellsystem%is_writable(a_result_writer) ) then
            call write_result(a_cellsystem, a_result_writer, conservative_variables_set, gradient_conservative_variables_set)
        end if
        ...
        do stage_num = 1, a_cellsystem%get_number_of_stages(a_time_stepper), 1
            call a_cellsystem%substitute_zeros(gradient_conservative_variables_set)
            call a_cellsystem%compute_gradient(a_gradient_calculator, conservative_variables_set, gradient_conservative_variables_set)

            call a_cellsystem%operate_cellwise(flux_set, gradient_conservative_variables_set, flux_calculation_operator)
