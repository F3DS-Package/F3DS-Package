module mp_weno5_js_module
    ...
    subroutine initialize(self, config)
        class(mp_weno_js   ), intent(inout) :: self
        class(configuration), intent(inout) :: config
        logical :: found

        call config%get_int("Reconstructor.MP-WENO5.Epsilon", self%epsilon_, found, 1.d-6)
        ...
        call config%get_int("Reconstructor.MP-WENO5.Alpha", self%alpha_, found, 10.d0)
        ...
        call config%get_int("Reconstructor.MP-WENO5.Beta", self%beta_, found, 4.d0)
    ...
    pure function reconstruct_lhc_interface( &
        ...
        class  (mp_weno_js), intent(in) :: self
