module class_weno5_z
    ...
    type, public, extends(reconstructor) :: weno5_z
        private
    ...
    subroutine initialize(self, config, a_reconstructor_generator)
        class(weno5_z                ),           intent(inout) :: self
        class(configuration          ),           intent(inout) :: config
        class(reconstructor_generator), optional, intent(inout) :: a_reconstructor_generator

        logical :: found

        call config%get_real("Reconstructor.WENO5-Z.Epsilon", self%epsilon_, found, 1.d-6)
        if(.not. found) call write_warring("'Reconstructor.WENO5-Z.Epsilon' is not found in configration you set. To be set dafault value.")
    ...
    pure function reconstruct_lhc( &
        ...
        do i = 1, num_primitive_variables, 1
            associate(                                                                                 &
                ...
                w(1:3) = compute_weights_left_side_z  (v_m2, v_m1, v, v_p1, v_p2, ideal_w, b_coef, self%epsilon_)
    ...
    pure function reconstruct_rhc( &
        ...
        do i = 1, num_primitive_variables, 1
            associate(                                                                                 &
                ...
                w(1:3) = compute_weights_right_side_z  (v_m2, v_m1, v, v_p1, v_p2, ideal_w, b_coef, self%epsilon_)
                p(1:3) = compute_polynomials_right_side(v_m2, v_m1, v, v_p1, v_p2, p_coef)
                reconstructed_primitive_variables(i) = w(1) * p(1) + w(2) * p(2) + w(3) * p(3)
            end associate
        end do
    end function reconstruct_rhc
end module class_weno5_z