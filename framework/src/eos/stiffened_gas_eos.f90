module class_stiffened_gas_eos
    ...
    subroutine initialize(self, a_configuration)
        ...
        do i = 1, self%num_phase_, 1
            call a_configuration%get_real   ("Phase.Phase property "//to_str(i)//".Specific heat ratio", specific_heat_ratio_(i), found)
            ...
            call a_configuration%get_real   ("Phase.Phase property "//to_str(i)//".Reference pressure", reference_pressure_(i), found)
    ...
    pure function compute_wood_soundspeed(self, pressure, densities, volume_fruction) result(soundspeed)
        class(stiffened_gas_eos), intent(in) :: self
        real(real_kind), intent(in) :: pressure
        ...
        if (volume_fruction < machine_epsilon) then
            squared_c2 = self%specific_heat_ratio_fluid_(2) * (pressure + self%reference_pressure_fluid_(2)) / densities(2)
            soundspeed = 1.d0 / densities(2) * squared_c2
        else if (1.d0 - machine_epsilon < volume_fruction) then
            squared_c1 = self%specific_heat_ratio_fluid_(1) * (pressure + self%reference_pressure_fluid_(2)) / densities(2)
            soundspeed = 1.d0 / densities(1) * squared_c1
        else
            squared_c1 = self%specific_heat_ratio_fluid_(1) * (pressure + self%reference_pressure_fluid_(1)) / densities(1)
            squared_c2 = self%specific_heat_ratio_fluid_(2) * (pressure + self%reference_pressure_fluid_(2)) / densities(2)
            soundspeed = volume_fruction / densities(1) * squared_c1 + (1.d0 - volume_fruction) / densities(2) * squared_c2
        end if
    end function compute_wood_soundspeed

    pure function compute_k(self, pressure, densities, volume_fruction) result(k)
        class(stiffened_gas_eos), intent(in) :: self
        real(real_kind), intent(in) :: pressure
        real(real_kind), intent(in) :: densities(:)
        real(real_kind), intent(in) :: volume_fruction
        ...
        if (volume_fruction < machine_epsilon) then
            ...
            squared_c1 = self%specific_heat_ratio_fluid_(1) * (pressure + self%reference_pressure_fluid_(1)) / densities(1)
            squared_c2 = self%specific_heat_ratio_fluid_(2) * (pressure + self%reference_pressure_fluid_(2)) / densities(2)
            k = volume_fruction * (1.d0 - volume_fruction) * (densities(1) * squared_c1 - densities(2) * squared_c2) &
              / (volume_fruction * densities(2) * squared_c2 + (1.d0 - volume_fruction) * densities(1) * squared_c1)
        end if
    ...
    pure function compute_mixture_specific_heat_ratio(self, volume_fruction) result(g)
        ...
        real(real_kind), intent(in) :: volume_fruction
        real(real_kind)             :: g
        g = 1.d0 + ((self%specific_heat_ratio_fluid_(1) - 1.d0) * (self%specific_heat_ratio_fluid_(2) - 1.d0)) &
                 / (volume_fruction * (self%specific_heat_ratio_fluid_(2) - 1.d0) + (1.d0 - volume_fruction) * (self%specific_heat_ratio_fluid_(1) - 1.d0))
    ...
    pure function compute_mixture_reference_pressure(self, volume_fruction, mixture_specific_heat_ratio) result(p)
        ...
        p = (mixture_specific_heat_ratio - 1.d0) / mixture_specific_heat_ratio &
          * self%compute_phi(volume_fruction)
    end function compute_mixture_reference_pressure

    pure function compute_phi(self, volume_fruction) result(pi)
        class(stiffened_gas_eos), intent(in) :: self
        real(real_kind), intent(in) :: volume_fruction
        real(real_kind)             :: phi
        phi =         volume_fruction  * self%specific_heat_ratio_fluid_(1) * self%reference_pressure_fluid_(1) / (self%specific_heat_ratio_fluid_(1) - 1.d0) &
            + (1.d0 - volume_fruction) * self%specific_heat_ratio_fluid_(2) * self%reference_pressure_fluid_(2) / (self%specific_heat_ratio_fluid_(2) - 1.d0)
