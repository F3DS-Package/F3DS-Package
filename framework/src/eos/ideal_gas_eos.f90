module class_ideal_gas_eos
    ...
    type, public, extends(eos) :: ideal_gas_eos
        private
    ...
    end type ideal_gas_eos

    contains

    subroutine initialize(self, a_configuration, an_eos_generator)
        class(ideal_gas_eos),           intent(inout) :: self
    ...
    pure function compute_pressure(self, specific_internal_energy, density, phase_num) result(pressure)
        class  (ideal_gas_eos), intent(in)           :: self
    ...
    pure function compute_isobaric_pressure(self, specific_internal_energy, densities, volume_fractions) result(pressure)
        ...
        pressure = (specific_internal_energy * self%compute_mixture_density(densities, volume_fractions)) / self%compute_gamma(volume_fractions)
    ...
    pure function compute_isobaric_internal_energy_density(self, pressure, densities, volume_fractions) result(energy_density)
        ...
        energy_density = self%compute_gamma(volume_fractions) * pressure
    ...
    pure function compute_soundspeed(self, pressure, density, phase_num) result(soundspeed)
        ...
        ! Protect square root of a negative number {@code max(pressure, 0.0_real_kind)} and zero division {@code max(density, machine_epsilon)}.
        soundspeed = sqrt(self%specific_heat_ratio_(n) * max(pressure, 0.0_real_kind) / max(density, machine_epsilon))
...
end module class_ideal_gas_eos