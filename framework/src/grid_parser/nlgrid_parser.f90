        ...
        do k = 1, self%num_ghost_cells_, 1
            do j = self%jmin, self%jmax, 1
                do i = self%imin, self%imax, 1
                    ...
                    centor_positions(n, 1) = self%x_poss_cell_cent(i, j, self%kmax) + 2.d0 * dble(k) * cell_to_face_vec(1)
                    centor_positions(n, 2) = self%y_poss_cell_cent(i, j, self%kmax) + 2.d0 * dble(k) * cell_to_face_vec(2)
                    centor_positions(n, 3) = self%z_poss_cell_cent(i, j, self%kmax) + 2.d0 * dble(k) * cell_to_face_vec(3)
