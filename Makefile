...
FRAMEWORK_SRCS+=$(wildcard $(FRAMEWORK_SRCDIR)/reconstructor/mp/*.f90)
...
FRAMEWORK_SRCS+=$(wildcard $(FRAMEWORK_SRCDIR)/face_gradient_calculator/*.f90)
# Tier 1 layer
...
# Diffusion Equation
DIFFUSION_EQUATION_TARGET=fdiffusion
DIFFUSION_EQUATION_SRCDIR=collection/diffusion_equation/src
DIFFUSION_EQUATION_SRCS+=$(wildcard $(DIFFUSION_EQUATION_SRCDIR)/*.f90)
DIFFUSION_EQUATION_OBJS=$(FRAMEWORK_OBJS)
DIFFUSION_EQUATION_OBJS+=$(subst .f90,.o, $(subst $(DIFFUSION_EQUATION_SRCDIR)/, $(OBJDIR)/, $(DIFFUSION_EQUATION_SRCS)))
...
$(ADVECTION_EQUATION_TARGET) $(DIFFUSION_EQUATION_TARGET) $(FIVE_EQUATION_MODEL_TARGET) $(VISCOUS_FIVE_EQUATION_MODEL_TARGET)
...
### Diffusion Equation
$(DIFFUSION_EQUATION_TARGET): $(DIFFUSION_EQUATION_OBJS)
	[ -d $(BINDIR) ] || mkdir -p $(BINDIR)
	$(FC) $(FCFLAGS) $(OMPFLAGS) $+ -o $(BINDIR)/$@
...
## Diffusion equation
$(OBJDIR)/%.o: $(DIFFUSION_EQUATION_SRCDIR)/%.f90
	[ -d $(dir $@) ] || mkdir -p $(dir $@)
	[ -d $(MODDIR) ] || mkdir -p $(MODDIR)
	$(FC) $(FCFLAGS) $(OMPFLAGS) -c $< -o $@
...
list:
	...
	@echo -e '\033[1;32m diffusion model objects:\033[0m'
	@echo -e $(DIFFUSION_EQUATION_OBJS)
